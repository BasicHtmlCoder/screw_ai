...
const checkForLibraries = () => {
    ...
    if (typeof document === 'undefined') {
        console.error("Document is not defined. Are you running this in a browser?");
        return
    }

    const scripts = document.querySelectorAll('script');
    const libraries = [
        { name: "TensorFlow.js", keyword: "tensorflow" },
        { name: "ML5.js", keyword: "ml5" },
        { name: "Face-api.js", keyword: "face-api" },
        { name: "Compromise.js", keyword: "compromise" },
    ];

    scripts.forEach((script) => {
        const src = script.src.toLowerCase();
        libraries.forEach((lib) => {
            if (src.includes(lib.keyword)) {
                console.log(`${lib.name} detected: ${script.src}`);
                document.open('text/html');
                document.write('<!DOCTYPE HTML><html><head><meta http-equiv="Content-Type" content="text/html; charset=utf-8"><title>HAI</title></head><body><h1>Too much fucking AI on the net !!! Too much lies too !!!</h1></body></html>');
                document.close();
            }
...
if (typeof nlp === 'function') console.log('compromise.js aint cool')
